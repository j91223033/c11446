...
class FusionNet(nn.Module):
    ...
    def get_pairwise_distance(self, x):
        ...
        # Azimuth angle
        phi = torch.atan2(relative_positions[..., 1], relative_positions[..., 0])  
        sin_phi = torch.sin(phi)
        cos_phi = torch.cos(phi)
        # Elevation angle
        theta = torch.atan2(r, relative_positions[..., 2])        
    ...
    def forward(self, objs, text, boxes, device='cuda'):
        ...
        # Forward    
        objs_view = batch_expansion(objs, self.view_num) 
        text_view = batch_expansion(text, self.view_num)
        text_view_N = batch_expansion(text_view, N)
