...
from in_out.data_generation.nr3d import decode_stimulus_string
from in_out.pt_datasets.utils import dataset_to_dataloader
from utils.models_utils import detailed_predictions_on_dataset
from utils.utils import pickle_data


def analyze_predictions(model, dataset, class_to_idx, pad_idx, device, config, out_file=None, visualize_output=True,tokenizer=None):
    """
    :param dataset:
    :param net_stats:
    :param pad_idx:
    ...
    test_seeds = [config.random_seed, 1, 10, 20, 100]
    net_stats_all_seed = []
    for seed in test_seeds:
        d_loader = dataset_to_dataloader(dataset, 'test', config.batch_size, n_workers=5, seed=seed)
        assert d_loader.dataset.references is references
        net_stats = detailed_predictions_on_dataset(model, d_loader, config=config, device=device, FOR_VISUALIZATION=True,tokenizer=tokenizer)
