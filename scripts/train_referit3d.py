...
from utils.models_utils import single_epoch_train, evaluate_on_dataset
...
def log_train_test_information(logits_train, logits_eval):
        ...
        logger.info('Train: category_top1: {:.3f}, category_top3: {:.3f}, category_top5: {:.3f}, spatial_top1: {:.3f}, spatial_top3: {:.3f}, spatial_top5: {:.3f}'.format(logits_train['category_top1'],
                                                            logits_train['category_top3'],
                                                            logits_train['category_top5'],
                                                            logits_train['spatial_top1'],
                                                            logits_train['spatial_top3'],
                                                            logits_train['spatial_top5'],))
        logger.info('Train: icategory_top1: {:.3f}, icategory_top3: {:.3f}, icategory_top5: {:.3f}, ispatial_top1: {:.3f}, ispatial_top3: {:.3f}, ispatial_top5: {:.3f}'.format(logits_train['icategory_top1'],
                                                            logits_train['icategory_top3'],
                                                            logits_train['icategory_top5'],
                                                            logits_train['ispatial_top1'],
                                                            logits_train['ispatial_top3'],
                                                            logits_train['ispatial_top5'],))
        logger.info('Test: category_top1: {:.3f}, category_top3: {:.3f}, category_top5: {:.3f}, spatial_top1: {:.3f}, spatial_top3: {:.3f}, spatial_top5: {:.3f}'.format(logits_eval['category_top1'],
                                                            logits_eval['category_top3'],
                                                            logits_eval['category_top5'],
                                                            logits_eval['spatial_top1'],
                                                            logits_eval['spatial_top3'],
                                                            logits_eval['spatial_top5'],))
        logger.info('Test: icategory_top1: {:.3f}, icategory_top3: {:.3f}, icategory_top5: {:.3f}, ispatial_top1: {:.3f}, ispatial_top3: {:.3f}, ispatial_top5: {:.3f}'.format(logits_train['icategory_top1'],
                                                            logits_eval['icategory_top3'],
                                                            logits_eval['icategory_top5'],
                                                            logits_eval['ispatial_top1'],
                                                            logits_eval['ispatial_top3'],
                                                            logits_eval['ispatial_top5'],))
...
if __name__ == '__main__':
    ...
    referit_data = load_referential_data(config, args.referit3D_file, scans_split)
    ...
    model = MiKASA_transformer(config, n_classes, class_name_tokens, ignore_index=pad_idx, bert_pretrain_path=args.bert_pretrain_path)
    ...
    if config.gpu_num > 1:
        param_list=[
            {'params':model.module.language_encoder.parameters(),'lr':config.init_lr*0.1},
            {'params':model.module.object_encoder.parameters(), 'lr':config.init_lr},
            {'params':model.module.post_obj_enc.parameters(), 'lr':config.init_lr*0.1},
            {'params':model.module.obj_feature_mapping.parameters(), 'lr': config.init_lr},
            {'params':model.module.box_feature_mapping.parameters(), 'lr': config.init_lr},
            {'params':model.module.fusion_net.parameters(), 'lr': config.init_lr*0.1},
            {'params':model.module.language_clf.parameters(), 'lr': config.init_lr},
            {'params':model.module.object_clf.parameters(), 'lr': config.init_lr},
            {'params':model.module.post_object_clf.parameters(), 'lr': config.init_lr},
            {'params':model.module.fusion_clf.parameters(), 'lr': config.init_lr},
        ]
    else:
        param_list=[
    ...
    if args.mode == 'train':
        ...
        with tqdm.trange(start_training_epoch, config.max_train_epochs + 1, desc='epochs') as bar:
            ...
            for epoch in bar:
                ...
                # Train:
                tic = time.time()
                train_meters, logits_analyze_all = single_epoch_train(model, data_loaders['train'], criteria, optimizer,
                                                  device, pad_idx, config=config, tokenizer=tokenizer,epoch=epoch)
                for key in logits_analyze_all:
                    logits_analyze_all[key] = logits_analyze_all[key].float().mean().item()
                toc = time.time()
                timings['train'] = (toc - tic) / 60

                # Evaluate:
                tic = time.time()
                test_meters, logits_analyze_all_eval = evaluate_on_dataset(model, data_loaders['test'], criteria, device, pad_idx, config=config, tokenizer=tokenizer)
                for key in logits_analyze_all_eval:
                    logits_analyze_all_eval[key] = logits_analyze_all_eval[key].float().mean().item()
                toc = time.time()
                ...
                log_train_test_information(logits_analyze_all, logits_analyze_all_eval)
        ...
        print('Post-Object-Clf-Accuracy: {:.4f}'.format(meters['test_post_object_cls_acc']))"""
