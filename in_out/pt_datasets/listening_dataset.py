...
class ListeningDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        res['objects'], res['objects_mask'], res['objects_color'] = pad_samples(samples, self.max_context_size)
...
def make_data_loaders(config, referit_data, vocab, class_to_idx, scans, mean_rgb, mode):
    ...
    object_transformation = partial(mean_rgb_unit_norm_transform, mean_rgb=mean_rgb,
                                    unit_norm=config.unit_sphere_norm)
    ...
    for split in splits:
        ...
        max_distractors = config.max_distractors if split == 'train' else config.max_test_objects - 1
        ...
        dataset = ListeningDataset(references=d_set,
                                   ...
                                   visualization=mode == 'evaluate')

        seed = None
        if split == 'test':
            seed = config.random_seed

        data_loaders[split] = dataset_to_dataloader(dataset, split, config.batch_size, n_workers, seed=seed)
